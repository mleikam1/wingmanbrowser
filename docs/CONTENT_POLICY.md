...
The local repository verifies Ed25519 signatures, catalog size/hash/schema/version/sequence, record count, IDs, exact asset paths, contexts, review dates, expiry, revocations, content byte limits and each body hash. A durable sequence/time/revocation checkpoint and an in-process monotonic clock prevent ordinary application rollback from undoing the last durably recorded floor. This is not a trusted wall-time service: OS clock manipulation between runs, particularly before the first checkpoint, can affect freshness; reinstallation, backup restoration or modification of local trust storage are outside that guarantee. A failed replacement retains a still-valid current snapshot. Missing/corrupt/expired policy or unavailable trust storage yields restricted Home. No error path converts to allow.
