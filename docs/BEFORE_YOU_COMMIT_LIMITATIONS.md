...
## UI handoff: explicit findings export

The updated screen uses shared page/status components, direct evidence panels, an actual cancel-check action, and an exact export preview. Native computation remains asynchronous; cancellation invalidates its eventual result rather than claiming to terminate work already running in the isolate.

`CommitReviewExport` validates the same bounded report schema, then exports the displayed finding statuses, excerpts, source label/type, check time and limitations. It does not export the full input, internal report ID, approved resource ID or fingerprint. Recognizable URL/email/credential/payment-number/control-character strings cause the affected label or excerpt to be replaced with an explicit omission marker. The finding's status is retained rather than inventing different evidence. This is incomplete detection: names, addresses and unlabeled secrets can still be present, so the user must review the exact preview.

Copying requires a second explicit action after preview and warns about other apps and device clipboard sync. Private findings are never saved through normal callbacks, but the user may explicitly copy a reviewed export outside that private view. Closing a preview, editing, route changes, backgrounding, source ineligibility or loss of the originating session invalidates pending UI outcomes. An OS clipboard operation already invoked cannot be rolled back; interrupted completion is recorded without a late success claim. Failed copy is described as unconfirmed, never submitted.

The journal's added `analysisExported` event contains only outcome and clipboard destination, not findings or source data. No cloud analysis, website access or autonomous merchant action was added. The UI milestone's focused tests and actual visual checks are recorded separately from the historical 0.5 results above.

[O/C UI evidence](ui/OFFICIAL_COMMIT_QA.md) records the setup, actual pending/cancel state, direct-evidence findings, and exact export preview in both themes. Practice examples in those captures are explicitly invented and analyzed by the real matcher. Clipboard writes are intercepted by test callbacks; no actual OS clipboard runtime claim is made.
