...
| Bookmark files | Import reads only a selected HTML file with bounded inert preview and confirmed merge. Export discloses full titles/addresses, then opens mobile sharing or a Web download; plaintext files and external share caches may remain |
| Reading list | Local SQLite metadata, limited to 500 items: title, address, created/read timestamps. Explicit save/add only; no page fetch or offline content archive. Saving private pages is disabled |
| Reader text | iOS-only explicit local extraction into transient plain text; no saved article, remote summarizer or active HTML |
| Normal tab metadata | Local SQLite; separate from live browser controllers |
| Preferences | Local SQLite; includes theme, chosen search provider, website size, Guard categories, custom site rules and Focus settings |
...
Private browsing changes storage behavior. Private pages do not enter Wingman's persistent history. Their tab metadata does not survive restart. On iOS, the private `WKWebsiteDataStore.nonPersistent()` is assigned before WKWebView creation and the native adapter verifies it is nonpersistent before loading a page. A local plugin patch exposes this pre-creation configuration and provides explicit controller teardown after native close/private-store clearing; native release is checked before shared data deletion. Changing a WebView's store after creation would not provide private isolation. See the [patch manifest](../third_party/webview_flutter_wkwebview/WINGMAN_PATCH.md) and current platform validation for the exact tested scope.
...
A small licensed third-party tracker list filters browser subresources without replacing publisher ads or uploading resource URLs. Its native browser adapter cannot inspect a third-party ad SDK's own views. This technical limitation is not a commercial privacy exemption: the separate ad policy withholds the current provider when protection requirements are strict or unknown, and any standard-mode provider remains subject to privacy review. Android exposes batched local blocked-resource counts; iOS reports counts as unavailable because its compiled content rules do not provide a reliable equivalent.
...
`AdPlacement` has only explicit owned placements. `AdPolicyService` rejects browser pages, private sessions, sensitive settings/support/security/Guard surfaces, route mismatches or covered/backgrounded Home, unknown/strict protection requirements, unverified provider/inventory, missing consent and missing opt-in. Eligibility is checked before UMP/SDK contact, not only after a creative arrives. Live local state notifications cancel same-route Guard/private changes before a frame, and asynchronous gates reread current state; missing live inputs deny requests. The ad component receives no URL, page title, content, search terms, history, location or user identifier. Do not register a browsing WebView with `MobileAds.registerWebView`; do not enable ad SDK web-content integration.

Advertisements are off by default. The mobile debug demo requires the build flag, eligible foreground Home with known standard protection requirements, a button tap, and UMP authorization before an ad request. Release builds cannot request ads. The native SDK remains linked in the mobile app even with the demo off; Wingman does not claim that linking, platform registration or an initialized SDK implies zero processing. Route coverage, foreground/private/protection changes or Home removal dispose a loaded demo ad and invalidate pending attempts. Returning to eligible Home requires a fresh tap; a previously started native SDK/form operation cannot always be recalled. The SDK has no general shutdown API; leaving Home is not a promise that all SDK background activity has ended. See [monetization policy](MONETIZATION_POLICY.md) and [demo setup](MONETIZATION.md).
...
The request explicitly selects non-personalized ads and configures a maximum `G` content rating before SDK initialization. A rating or category filter does not prove strict Guard compatibility; that inventory is withheld. This does not mean no cookies, no identifiers, no technical processing or no consent requirement. UMP and regional configuration remain necessary. [Google explanation of non-personalized ads](https://support.google.com/admob/answer/7676680)
