...
Recognized URL-like input and unsupported schemes produce a fixed unavailable message. Other URL-like strings may produce empty local results; neither path starts a request or falls back to a web provider. Unmatched ordinary text produces an empty local result set. Queries involving educational/support topics are not blocked simply because a word could be sensitive; only exact approved results can appear.
