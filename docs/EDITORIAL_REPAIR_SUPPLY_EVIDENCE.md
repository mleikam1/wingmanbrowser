...
The related synthetic widget regressions passed **22/22** (`test/ui/editorial_delivery_widgets_test.dart` and `test/ui/live_content_widgets_test.dart`), checking optional-image resilience, stable IDs/scroll, denied rights, canonical links and 200% text/semantic activation. Those tests establish behavior, not live editorial supply. Real-device checks, the final compiled build and deployment status belong to the main delivery report.
