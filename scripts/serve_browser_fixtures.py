...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/resource-checks':
            self.page('Redirect resource check', '<p>First add httpbingo.org as an additional blocked domain in Wingman. This neutral fixture tests an allowed initial resource URL that redirects to that blocked domain.</p><p id="status">Image not requested</p><img id="image" alt="Redirect result" style="max-width:220px"><button onclick="const i=document.getElementById(\'image\');i.onload=()=>document.getElementById(\'status\').textContent=\'Redirected image loaded\';i.onerror=()=>document.getElementById(\'status\').textContent=\'Redirected image unavailable\';i.src=\'/resource-redirect?run=\'+Date.now()">Load redirected image</button><a href="https://httpbingo.org/html">Blocked top-level positive control</a>'); return
        if path == '/category-checks':
            ...
            self.page('Synthetic policy checks', '<p>Reserved .test destinations. Each must be blocked before network use by the installed baseline, with no live prohibited content.</p>'+links); return
        if path == '/scroll':
