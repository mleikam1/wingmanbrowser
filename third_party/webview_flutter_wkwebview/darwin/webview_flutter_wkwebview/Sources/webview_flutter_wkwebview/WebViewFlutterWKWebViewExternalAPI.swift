...
public class FWFWebViewFlutterWKWebViewExternalAPI: NSObject {
  ...
  /// Wingman extension for selecting a nonpersistent store before view creation.
  public static func wingmanConfiguration(
    forIdentifier identifier: Int64, registrar: FlutterPluginRegistrar
  ) -> WKWebViewConfiguration? {
    guard let plugin = registrar.valuePublished(byPlugin: "WebViewFlutterPlugin")
      as? WebViewFlutterPlugin else { return nil }
    return plugin.proxyApiRegistrar?.instanceManager.instance(forIdentifier: identifier)
  }
